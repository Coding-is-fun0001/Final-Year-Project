...
def _safe_get_exe_path(pid: int) -> Optional[str]:
    """Best-effort psutil exe() lookup for whitelist path verification.
    Returns None if the pid is unknown/gone/inaccessible -- callers treat
    that as "can't verify" and fall back to name-only matching."""
    if not pid:
        return None
    try:
...
def get_whitelist_tier(proc_name: str, config: dict, exe_path: str = None) -> str:
    """3-layer process trust classification (score-adjustment input only
    -- see adjust_tier_for_whitelist for why this never becomes a hard
    bypass). Layer 1: OS-level trusted processes. Layer 2: user-
    configured trusted applications. Anything else: full monitoring.

    A name-only match for tier1 is NOT enough: a malicious binary simply
    named "svchost.exe" and run from anywhere (e.g. Temp) would otherwise
    inherit tier1's strong risk reduction. When exe_path is available, a
    tier1 name match is only honored if the executable actually resides
    under one of whitelist_tier1_trusted_dirs; otherwise it falls back to
    "none" (full monitoring). If exe_path could not be determined (e.g.
    the process already exited), we fall back to name-only matching --
    this only affects the SIZE of the reduction, never whether the
    activity is detected at all."""
    if not proc_name:
        return "none"
...
def adjust_tier_for_whitelist(base_tier: str, proc_name: str, config: dict, exe_path: str = None) -> str:
    """Whitelist as a RISK-SCORE ADJUSTMENT applied at final
    classification, never an early-exit bypass:
      if process in whitelist: risk_score -= small_value   (correct)
      if process in whitelist: safe                        (WRONG -- a
        compromised/injected trusted process, or a legitimate app
        scripted into doing real damage, is still a real attack)

    "critical" is never adjusted -- a honeypot touch or a confirmed
    encrypt+delete sequence is exactly as damning coming from svchost.exe
    as from an unknown process (arguably more so). tier1 (strong
    reduction) demotes "high" to "low": never silently auto-act against
    an OS process, always ask a human first. tier2 (mild reduction)
    only quiets the WEAKEST tier ("low" -> "quiet", dropped after an
    audit-log line instead of surfacing an ask-first alert) since that's
    where routine legitimate activity from a known app would otherwise
    generate the most noise; "high" stays "high" for tier2 -- a trusted
    app doing something that looks like an encryption chain is still
    exactly as suspicious as it would be from anything else."""
...
def get_file_entropy_sampled(file_path: str) -> float:
    """Multi-point sampling — same total I/O cost as a plain head/mid/tail
    split (~1.5MB), but spread across more, evenly-spaced offsets. A fixed
    3-point sample is trivially evadable by "sparse" encryption that just
    avoids those three exact spots (e.g. only touches the middle 10% of
    the file); more, evenly-spread sample points make that much harder
    without materially increasing the amount of data read."""
    try:
        ...
        num_points = 6
        chunk = 256 * 1024  # 6 x 256KB ~= the previous 3 x 512KB total
        data = bytearray()
        with open(file_path, "rb") as f:
...
class ProcessTerminator:
    ...
    def lock_monitor_folder(monitor_path: str) -> int:
        """Best-effort containment for the case where the responsible
        process could not be identified (pid=0), so it can't be suspended
        or terminated directly: strip write permission from every file
        under monitor_path, and from the directory itself, so further
        writes/renames (i.e. further encryption) fail. Reversible by
        restoring normal permissions afterwards. Returns the number of
        files locked."""
        locked = 0
    ...
    def unlock_monitor_folder(monitor_path: str) -> int:
        """Undo lock_monitor_folder: restores normal read/write permissions.
        Cheap and safe to call even if the folder was never locked. Exists
        so an auto-lock triggered by a false positive costs the user a
        few seconds of blocked writes, not a stuck folder."""
        unlocked = 0
        root = Path(monitor_path)
        try:
...
class PerPidStore:
    ...
    def mark_entropy_anomaly(self, pid):
        """Bumps the anomaly counter for an already-recorded event, without
        re-appending to q_1s/q_1h/q_24h/action_seq (that already happened
        once via record() for this same event -- calling record() again
        here would double-count it in every frequency/sequence check)."""
        with self.lock:
            if pid in self.history:
                self.history[pid]["entropy_anomaly_count"] += 1

    def mark_entropy_pending(self, pid, file_path, delta=None):
        """Entropy detection is an ASSIST to file-operation detection, not
        an independent alerter: this just records that `file_path` looked
        anomalous when pid touched it, and (if given) logs the signed
        delta for trend tracking. File-operation detection is what decides
        whether/when this evidence actually confirms an attack -- see
        get_entropy_pending_count() / resolve_entropy_pending(), used at
        delete time."""
        with self.lock:
            if pid in self.history:
    ...
    def resolve_entropy_pending(self, pid):
        """Clears the pending set after file-operation detection has
        confirmed and fired on it, so the SAME evidence doesn't keep
        re-triggering forever on every subsequent unrelated delete from
        this pid -- fresh anomalies accumulated afterwards can still
        trigger again legitimately."""
        with self.lock:
            if pid in self.history:
                self.history[pid]["entropy_pending_files"].clear()

    def get_entropy_trend(self, pid):
        """Returns the recent (signed) entropy deltas for this pid, most
        recent last -- used as the "directionality" component of the
        hardware behavior signature (see RiskScorer.process_hardware_event):
        a consistently positive/increasing trend across many files is a
        much stronger signal than any single sample."""
        with self.lock:
            if pid not in self.history:
                return []
    ...
    def get_touched_file_count(self, pid):
        """"Repetition" component of the hardware behavior signature --
        how many distinct files this pid has touched recently."""
        with self.lock:
            if pid not in self.history:
                return 0
    ...
    def detect_encryption_sequence(self, pid, min_pairs=3):
        ...
        typically performs a tight Read->Write(encrypt)->Rename loop across
        many files, OR the equivalent achieved without ever calling
        rename(): read the original, write the encrypted content to a
        brand-new path, then delete the original (this is what a plain
        open()/write()/unlink() implementation looks like, and is exactly
        what a delete+create pair below is watching for). We approximate
        both variants purely from watchdog event types:
          - a 'modified'/'created' event immediately followed by a
            'moved' (rename) or another 'modified' event, OR
          - a 'created'/'deleted' event immediately followed by the other
            of the two (write-new-file-then-delete-original, in either
            order depending on which watchdog event lands first)
        repeated several times in a short window."""
...
class ForensicEngine:
    ...
    def _build_narrative_report(self, evidence: dict, json_path: Path) -> str:
        """Turns the raw evidence dict into a professional, human-readable
        incident report -- narrative sections a non-technical user can
        follow (what happened, which detector caught it, how the attack
        chain correlates), instead of a JSON dump."""
        sep, sub = "=" * 70, "-" * 70
        lines = [
            sep,
    ...
    def record_decision(self, pid, proc_name, reason, recent_files, decision):
        """Lightweight timeline/evidence record of the user's decision --
        independent of, and much cheaper than, generate_evidence_summary's
        full capture. Used for the "trust" path where no heavy process
        forensics are warranted (nothing was terminated), but the
        decision itself still needs to be on the record for audit."""
        record = {
            ...
            try:
                ...
                if confirm_until is not None:
                    if abnormal:
                        ...
                        confirm_until = None  # window expired, unconfirmed
...
class FileOperationMonitor(FileSystemEventHandler):
    ...
    def _get_calling_process(self, file_abs):
        ...
        if found_pid is None and fallback_pid is not None:
            logging.debug(
                ...
                f"via cwd+recency heuristic (no exact open-handle match)."
            )
    ...
    def _analyse(self, f_path, event_type):
        ...
        # (Honeypot is handled unconditionally at the top of this method,
        # before the path-whitelist check -- see there.)
    ...
    def _describe_honeypot_change(self, f_abs, event_type):
        """Every honeypot touch is already unconditionally treated as a
        violation (no legitimate process ever has a reason to touch one)
        -- this just adds forensic detail about WHAT changed (name, hash/
        content, entropy) on top of THAT it changed, using the baseline
        hash/entropy captured for every file (honeypots included) at
        startup."""
...
class ControlPanel:
    ...
    def _handle_tk_callback_exception(self, exc, val, tb):
        """Installed as root.report_callback_exception: log instead of
        letting a single bad callback's traceback only go to stderr."""
        logging.error(
    ...
    def _open_latest_forensic_report(self):
        """Manual "View Latest Forensic Report" button -- shows the most
        recently generated narrative report (see
        ForensicEngine._build_narrative_report)."""
        try:
            Path(FORENSIC_DIR).mkdir(parents=True, exist_ok=True)
            reports = sorted(
    ...
    def _show_forensic_report_window(self, report_text, report_path):
        """Displays a narrative forensic report (see
        ForensicEngine._build_narrative_report) in a read-only popup,
        with the on-disk report path shown clearly."""
        if not report_text:
            report_text = (
    ...
    def _on_unlock_folder(self):
        """Manual escape hatch for an auto-lock triggered by a false
        positive: restores normal read/write permissions on the monitored
        folder. Safe to click even if nothing is currently locked."""
        try:
            ...
            logging.error(f"Failed to unlock monitor folder: {e}")
            messagebox.showerror("Aegis Shield - Error", f"Unlock failed: {e}")
    ...
    def _ask_trust_dialog(self, title, message):
        """Trust-confirmation prompt with explicit 'TRUST' / 'NOT TRUST'
        buttons instead of generic Yes/No -- removes any ambiguity about
        which choice does what. Returns True for "NOT TRUST" (confirm the
        threat: terminate + report) and False for "TRUST" (dismiss: resume
        + unlock), matching the boolean this method's callers previously
        got back from messagebox.askyesno. Still uses grab_set/wait_window
        so the Tk event loop keeps pumping while the dialog is open."""
        ...
        # Closing the dialog (X button) defaults to the protective choice,
        # same as the old messagebox default="yes".
    ...
    def _process_alert_safely(self, alert):
        try:
            ...
            try:
                ...
                ans = True  # default to the protective choice if the dialog itself fails
    ...
    def _finish_alert(self, alert, ans, forensic_thread, start_time):
        try:
            ...
            if ans:
                ...
                # Trusted -- dismiss protection entirely.
                if pid != 0:
    ...
    def _on_quit_clicked(self):
        """Handles both the QUIT button and the window's close (X)
        button -- either path requires an explicit double-confirmation
        before monitoring actually stops, so a user can't lose protection
        by accidentally clicking/closing the window."""
        if self._closing:
            return
        try:
    ...
    def on_closing(self):
        ...
        # Stop the event-analysis worker pool (queued events are dropped,
        # not drained, so shutdown doesn't hang behind a large backlog)
        try:
