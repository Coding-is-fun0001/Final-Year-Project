...
# Alert reasons matched here are treated as near-zero false-positive
# signals: a legitimate process/user essentially never produces them
# (touching a decoy honeypot file, an Office document losing its magic
# bytes, a burst of 5+ file ops in the same second, a rename to a known
# ransomware extension, registry persistence/anti-recovery tampering...).
# These are acted on immediately instead of waiting for a user's yes/no
# click, because by the time a person manually confirms a fast multi-file
# encryption burst the damage is usually already done. Lower-confidence
# signals (a single file's entropy delta, an unconfirmed hardware spike)
# still ask first, since legitimate bulk operations (backups, indexing,
# batch conversions) can look similar, and auto-containment on those would
# just be disruptive false-positive noise.
_HIGH_CONFIDENCE_KEYWORDS = (
    "honeypot", "header mismatch", "encryption sequence",
    "autorun", "shadow copy", "1s burst", "suspicious extension",
...
def is_high_confidence_alert(reason: str) -> bool:
    ...
    return any(k in reason_l for k in _HIGH_CONFIDENCE_KEYWORDS)
...
class FileOperationMonitor(FileSystemEventHandler):
    ...
    def __init__(self, monitor_path, config, forensic,
        ...
        # Event ingestion is decoupled from analysis: watchdog's dispatch
        # thread calls on_modified/on_created/on_moved/on_deleted, which
        # must return almost instantly (just enqueue) so a burst of many
        # near-simultaneous file changes (e.g. mass encryption) doesn't
        # queue up behind the expensive per-file work (full process-list
        # scan for attribution, hashing, entropy sampling). A small pool of
        # worker threads drains the queue and does that expensive work in
        # parallel instead of serially on the single watchdog thread.
    ...
    # --- watchdog callbacks: enqueue only, return immediately ---------
    # These run on watchdog's own dispatch thread. If they did the actual
    # (expensive) analysis inline, a burst of many near-simultaneous file
    # changes would back up behind that thread, and by the time analysis
    # caught up the attack would already be finished. Keeping these as a
    # cheap queue.put() means the OS-level file-change notifications keep
    # draining as fast as watchdog can read them; the real work happens on
    # the worker pool below, in parallel with more events still arriving.
...
class ControlPanel:
    def __init__(self, root, config, monitor_path, forensic,
        ...
        # Auto-response bookkeeping for a burst of many high-confidence
        # alerts (e.g. one per encrypted file): the containment action
        # itself (lock the folder) only needs to happen once, and the
        # follow-up "here's what happened" popup is a *blocking* modal —
        # showing one per alert would mean clicking through potentially
        # hundreds of dialogs before the queue ever drains, which looks
        # exactly like a freeze/crash under a large attack. Both are
        # coalesced: repeat the containment log line, but only actually
        # touch the filesystem / show a popup once per incident.
        self._folder_locked            = False
        self._last_auto_notice_time    = 0.0
        self._auto_notice_cooldown_sec = 15
    ...
    def _create_widgets(self):
        ...
        self.unlock_btn = tk.Button(
            self.btn_frame,
            text="🔓 解锁监控文件夹 (Unlock Monitored Folder)",
            font=("Arial", 9, "bold"),
            bg="#2a2a35", fg="#ffcc00",
    ...
    def _on_unlock_folder(self):
        ...
        try:
            ...
            messagebox.showerror("Aegis Shield - 错误", f"解锁失败: {e}")
    ...
    def _process_alert_safely(self, alert):
        try:
            ...
            high_confidence = is_high_confidence_alert(reason)
            ...
            if pid == 0:
                ...
                self._do_log(
                    ...
                    "Capturing forensic evidence for the affected files...\n",
                    "warning")
                ...
                if high_confidence:
                    # Near-zero false-positive signal (honeypot, header
                    # mismatch, burst, etc): act immediately instead of
                    # waiting for a yes/no click — a fast encryption burst
                    # is usually already finished by the time a human
                    # could respond. Folder-lock is cheap to undo (see
                    # "Unlock Monitored Folder" button) if this does turn
                    # out to be a false positive.
                    #
                    # No join-before-act here: unlike killing a process,
                    # locking files doesn't destroy any volatile evidence,
                    # so there's nothing to wait on. This matters a lot
                    # under a real burst — a distinct alert fires per
                    # affected file, so with hundreds of files this branch
                    # can run hundreds of times; blocking 5s per alert here
                    # would itself look like a freeze.
                    if not self._folder_locked:
                        locked = ProcessTerminator.lock_monitor_folder(self.monitor_path)
                        self._folder_locked = True
                        self._do_log(
                            f"🔒 [AUTO-LOCKED] High-confidence signal — monitored "
                            f"folder frozen read-only ({locked} files): "
                            f"{self.monitor_path}\n", "critical")
                    else:
                        self._do_log(
                            f"🔒 [AUTO-CONTAINED] {reason} (folder already locked)\n",
                            "critical")

                    # The action already happened above with no user input
                    # needed; this popup is purely informational. Under a
                    # burst, dozens/hundreds of these alerts can arrive
                    # within milliseconds of each other — showing one
                    # blocking popup per alert would mean clicking through
                    # all of them before the queue ever drains, which is
                    # itself indistinguishable from a hang. So only surface
                    # one every _auto_notice_cooldown_sec; the rest are
                    # still fully logged and acted on, just without a popup.
                    now = time.time()
                    if now - self._last_auto_notice_time >= self._auto_notice_cooldown_sec:
                        self._last_auto_notice_time = now
                        try:
                            messagebox.showinfo(
                                "Aegis Shield - Auto-Contained",
                                f"High-confidence threat signal detected!\n\n"
                                f"Reason : {reason}\n"
                                f"Files  : {len(recent_files)}\n\n"
                                f"The responsible process could not be identified, "
                                f"so the monitored folder was locked read-only "
                                f"immediately as a precaution. Further alerts in "
                                f"this incident are logged but won't each show a "
                                f"popup.\n\n"
                                f"Use 'Unlock Monitored Folder' to restore normal "
                                f"access once you've reviewed the forensic report.")
                        except Exception as e:
                            logging.debug(f"showinfo failed: {e}")
                    return
                ...
                try:
                    ans = messagebox.askyesno(
                        ...
                        (f"Suspicious bulk activity detected!\n\n"
                         ...
                         f"cannot be suspended/terminated directly.\n\n"
                         f"YES → LOCK the monitored folder (block further writes) "
                         f"while evidence is preserved\n"
                         f"NO  → Take no action, just record this event in the log"),
                ...
                if ans:
                    ...
                    locked = ProcessTerminator.lock_monitor_folder(self.monitor_path)
                    self._do_log(
                        f"🔒 [LOCKED] Monitored folder frozen read-only "
                        f"({locked} files): {self.monitor_path}\n", "critical")
                else:
                    self._do_log(
                        "[RECORDED] No action taken — event logged only.\n", "warning")
                return
            ...
            if high_confidence:
                # Near-zero false-positive signal: don't wait on a yes/no
                # click — terminate immediately, the same way a real EDR
                # product would, then inform the user afterwards. Waiting
                # for manual confirmation on a signal this strong just
                # gives a fast encryption loop more time to finish.
                forensic_thread.join(timeout=5.0)
                ...
                # Same reasoning as the pid=0 branch: under a burst, a
                # separate alert can fire per affected file, so this
                # branch may run many times for the same or successive
                # pids. Acting (terminate/quarantine) happens every time,
                # but the blocking popup is throttled so the queue can
                # still drain without needing a click per alert.
                now = time.time()
                if now - self._last_auto_notice_time >= self._auto_notice_cooldown_sec:
                    self._last_auto_notice_time = now
                    try:
                        messagebox.showinfo(
                            "Aegis Shield - Auto-Terminated",
                            f"High-confidence threat signal detected!\n\n"
                            f"Process : {proc_name} (PID: {pid})\n"
                            f"Reason  : {reason}\n"
                            f"Files   : {len(recent_files)}\n\n"
                            f"The process was terminated and quarantined "
                            f"immediately as a precaution. Further alerts in "
                            f"this incident are logged but won't each show a "
                            f"popup. Review the forensic report for details.")
                    except Exception as e:
                        logging.debug(f"showinfo failed: {e}")
                return
