    ...
    def generate_evidence_summary(self, pid: int, proc_name: str, reason: str, recent_files: list):
        ...
        for f_path in recent_files:
            try:
                ...
                # A single file racing with a still-running attacker (moved/
                # deleted/locked between the exists() check and the read)
                # must not abort the whole report -- under a large, fast
                # burst this used to kill the entire background thread
                # partway through, silently leaving NO report at all for
                # the incident.
...
class FileOperationMonitor(FileSystemEventHandler):
    ...
    def __init__(self, monitor_path, config, forensic,
        ...
        # Files already confirmed tampered (header mismatch or entropy
        # anomaly already fired/marked for this exact path) -- once a
        # file is known-bad, its content is permanently corrupted and
        # will keep failing the same header/entropy check forever. Any
        # further touch to it (AV scan, thumbnail/indexer, even a failed
        # write attempt by the original attacker) would otherwise keep
        # re-firing a fresh alert -- often attributed to a different,
        # unrelated pid each time, which bypasses the per-pid cooldown
        # entirely and looks like an endless stream of new warnings for
        # damage that was already reported once.
        ...
        try:
            ...
            for idx, proc in enumerate(candidates):
                ...
                try:
                    ...
                    # cwd only matches when the attacker process was
                    # actually LAUNCHED from inside the monitored folder.
                    # In practice a script is far more often run from
                    # wherever it lives (Desktop, a tools folder) and
                    # just passed the monitored folder as an argument
                    # (e.g. `python attack.py --target C:\...\test_monitor`)
                    # -- cwd alone never catches that case, which is why
                    # attribution kept coming back "Unknown" in testing.
                    # Command-line arguments persist for the process's
                    # whole life (unlike a file handle that's only open
                    # for a fraction of a second around each read/write),
                    # so check whether the monitored path shows up there
                    # too. Stronger, less coincidental signal than cwd,
                    # so it gets a more generous age window to tolerate
                    # queue backlog under a large burst.
                    if fallback_pid is None:
                        age = now - (proc.info.get("create_time") or 0)
                        if 0 <= age < 30:
                            try:
    ...
    def _analyse(self, f_path, event_type):
        ...
        if already_known_bad and not is_suspicious:
            # Already confirmed compromised -- a further touch to this
            # SAME file (AV scan, thumbnail/indexer, a failed write retry
            # by an already-terminated attacker) adds no new forensic
            # value. Without this, repeatedly recording it would keep
            # feeding the frequency/sequence-pattern detectors too (not
            # just header/entropy), eventually re-tripping a fresh alert
            # purely from being looked at again, not from any new damage.
            # A rename to a suspicious extension is still let through --
            # that IS new information even for an already-known-bad file.
            return
    ...
    def _fire_alert(self, pid, name, reason, f_abs):
        ...
        # For a known pid, the cooldown key is just the pid -- every file
        # it touches collapses into the same 60s slot, which is exactly
        # what stops one alert per affected file during a single burst.
        # For pid=0 (unattributed), the key used to be the FULL reason
        # string -- but reason strings embed the specific filename (e.g.
        # "File header mismatch: report3.docx ..."), so every different
        # file got its own cooldown slot and none of them collapsed. If
        # a burst of many files all end up unattributed (which happens
        # more often than a single known pid, since attribution can fail
        # independently per file), that meant one popup per file instead
        # of one for the whole incident. Use just the reason CATEGORY
        # (the part before the first ":") so same-type alerts collapse
        # together regardless of which file triggered them, while
        # different alert types (e.g. header mismatch vs bulk deletion)
        # still get their own slot as originally intended.
...
class ControlPanel:
    ...
    def _on_unlock_folder(self):
        ...
        # unlock_monitor_folder walks every file under monitor_path with a
        # chmod call each -- fine for a handful of files, but a large or
        # unexpectedly ballooned folder (e.g. an attack that kept creating
        # new files faster than it could be fully contained) can make this
        # take long enough, run synchronously on the Tk main thread, to
        # look like the whole window has frozen/crashed. Do the actual
        # file walk in the background and only touch the UI afterwards.
    ...
    def _on_unlock_finished(self, unlocked, error):
        ...
        if getattr(self, "file_monitor", None) is not None:
            ...
            # The incident is being dismissed -- if the user has restored
            # damaged files (from the shadow copy, a backup, etc.), don't
            # keep treating them as permanently known-bad. Any file that's
            # STILL actually tampered will simply get re-flagged fresh on
            # its next touch, same as any other file.
            ...
            if ans:
                ...
                if pid != 0:
                    ...
                    self._do_log(
                        ...
                        "success")
                    # "Trust" is a decision about THIS alert, not a free
                    # pass for this pid until the alert-cooldown expires.
                    # Without this, a resumed process that keeps doing
                    # something suspicious could be silently waved through
                    # for up to another 60s -- the cooldown exists to stop
                    # duplicate alerts for the SAME ongoing incident, not
                    # to suppress re-evaluation after the user has already
                    # made a fresh decision.
                    if getattr(self, "file_monitor", None) is not None:
