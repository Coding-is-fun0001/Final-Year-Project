...
def safety_check(target: Path):
    ...
    # Any subfolder (e.g. C:\Users\you\Downloads\ransom_test) is fine --
    # that's exactly where a normal user's test folder lives. Using "in
    # resolved.parents" here would incorrectly reject every subfolder under
    # home, since home is a parent of virtually all user-created paths.
    try:
...
def cmd_run(target: Path):
    ...
    print("\n⏸️  IMPORTANT: This process is now PAUSING to keep itself alive")
    print("   so the detector can properly suspend/kill it for testing.")
    print("   Press Ctrl+C when you're done testing, or wait ~60 seconds.\n")
    
    # Keep the process alive so detector can suspend/kill it
    # Otherwise the detector can't interact with the already-exited process
    try:
