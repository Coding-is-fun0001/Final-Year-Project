...
def is_whitelisted(pid: int, proc_name: str, file_path: str, config: dict) -> bool:
    ...
    try:
        ...
        for wp in w_paths:
            if abs_file.is_relative_to(Path(wp).resolve()):
                return True
    except Exception:
        pass
    return False
    ...
    def _analyse(self, f_path, event_type):
        ...
        try:
            ...
            for wp in self.config.get("white_list_paths", []):
                if abs_p.is_relative_to(Path(wp).resolve()):
                    return
        except Exception:
            pass
        ...
        if is_monitored and event_type in ("modified", "created") and os.path.exists(f_abs):
            try:
                ...
                if base_hash and curr_hash and curr_hash != base_hash and is_entropy_anomaly:
                ...
                if not base_hash and is_entropy_anomaly:
                    self.risk_scorer.report_event(
                        pid, name, f_abs, "entropy_anomaly"
                    )
